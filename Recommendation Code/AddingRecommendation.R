...
print_rankings <- function(data, rank_col, score_col, recommendation_col, title) {
  cat("\n")
  cat(paste0(rep("=", 70), collapse = ""), "\n")
  cat(title, "\n")
  cat(paste0(rep("=", 70), collapse = ""), "\n")
  cat(sprintf("%-5s %-25s %-15s %s\n", "#", "Town", score_col, "Recommendation"))
  cat(sprintf("%-5s %-25s %-15s %s\n", "", "<chr>", "<dbl>", "<chr>"))
  cat(paste0(rep("-", 70), collapse = ""), "\n")
  
  for(i in 1:nrow(data)) {
    cat(sprintf("%-5d %-25s %-15.2f %s\n", 
                i, 
                data[[rank_col]][i], 
                data[[score_col]][i],
                data[[recommendation_col]][i]))
  }
  cat("\n")
}
...
house_rankings <- house_prices %>%
  ...
  mutate(
    ...
    # Add recommendation: all affordable towns in top 10
    Recommendation = "Affordable"
  ) %>%
  arrange(desc(HouseScore)) %>%
  select(Town, HouseScore, avg_price, Recommendation) %>%
  head(10)

print_rankings(house_rankings, "Town", "HouseScore", "Recommendation", "TOP 10 TOWNS BY HOUSE SCORE (Affordability)")

# Also print with average price for reference
cat("\nDetailed House Prices:\n")
print(house_rankings, n = 10)
...
broadband_rankings <- broadband_with_town %>%
  ...
  summarise(
    ...
    avg_upload = mean(AvgUpload, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  mutate(
    ...
    # Add recommendation based on speed thresholds
    Recommendation = case_when(
      avg_download < 30 ~ "Poor",
      avg_download >= 30 & avg_download < 50 ~ "Moderate",
      avg_download >= 50 ~ "Good",
      TRUE ~ "Needs Improvement"
    )
  ) %>%
  arrange(desc(BroadbandScore)) %>%
  select(Town, BroadbandScore, avg_download, avg_upload, Recommendation) %>%
  head(10)

print_rankings(broadband_rankings, "Town", "BroadbandScore", "Recommendation", "TOP 10 TOWNS BY BROADBAND SCORE")

# Also print with speeds for reference
cat("\nDetailed Broadband Speeds:\n")
print(broadband_rankings, n = 10)
...
crime_rankings <- crime %>%
  ...
  mutate(
    ...
    # Add recommendation based on crime levels
    Recommendation = case_when(
      crime_count < 100 ~ "Safe",
      crime_count >= 100 & crime_count < 1000 ~ "Moderate",
      crime_count >= 1000 ~ "Needs Attention",
      TRUE ~ "Safe"
    )
  ) %>%
  arrange(desc(CrimeScore)) %>%
  select(Town = lsoa_name, CrimeScore, total_crimes = crime_count, Recommendation) %>%
  head(10)

print_rankings(crime_rankings, "Town", "CrimeScore", "Recommendation", "TOP 10 TOWNS BY CRIME SCORE (Safety)")

# Also print with crime counts for reference
cat("\nDetailed Crime Data:\n")
print(crime_rankings, n = 10)
...
# Export rankings with recommendations
write_csv(house_rankings %>% select(Town, HouseScore, avg_price, Recommendation), 
          paste0(output_dir, "/top10_house_score.csv"))
write_csv(broadband_rankings %>% select(Town, BroadbandScore, avg_download, avg_upload, Recommendation), 
          paste0(output_dir, "/top10_broadband_score.csv"))
write_csv(crime_rankings %>% select(Town, CrimeScore, total_crimes, Recommendation), 
          paste0(output_dir, "/top10_crime_score.csv"))

# Only export alternative crime rankings if it exists
if (exists("crime_rankings_alt")) {
  write_csv(crime_rankings_alt %>% select(Town, CrimeScore, total_crimes, Recommendation), 
            paste0(output_dir, "/top10_crime_score_by_town.csv"))
}
...
cat("================================================\n")
cat("Files created:\n")
cat("  - top10_house_score.csv\n")
cat("  - top10_broadband_score.csv\n")
cat("  - top10_crime_score.csv\n")
if (exists("crime_rankings_alt")) {
  cat("  - top10_crime_score_by_town.csv\n")
}
cat("\n")
...
crime_rankings_alt <- crime_with_town %>%
  ...
  mutate(
    ...
    # Add recommendation based on crime levels
    Recommendation = case_when(
      crime_count < 500 ~ "Safe",
      crime_count >= 500 & crime_count < 5000 ~ "Moderate",
      crime_count >= 5000 ~ "Needs Attention",
      TRUE ~ "Safe"
    )
  ) %>%
  arrange(desc(CrimeScore)) %>%
  select(Town, CrimeScore, total_crimes = crime_count, Recommendation) %>%
  head(10)

cat("\n")
cat("Alternative Crime Rankings (by extracted town name):\n")
print_rankings(crime_rankings_alt, "Town", "CrimeScore", "Recommendation", "TOP 10 TOWNS BY CRIME SCORE - Alternative Method")

# Also print with crime counts for reference
cat("\nDetailed Crime Data (Alternative):\n")
print(crime_rankings_alt, n = 10)